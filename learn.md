...
GitHub 仓库是 [lastadreamer/ai-chess](https://github.com/lastadreamer/ai-chess)。包名仍是 `xiangqi-engine` 0.3.0。象棋规则在 C++17 + pybind11；网络 / MCTS / 训练 / 网页对弈在 Python。五子棋规则全是 Python，和象棋共用同一条训练环。入库的五子棋门控权重在 `checkpoints/gomoku/best.pt`（约第 448 轮）。
...
- 入库的 `checkpoints/gomoku/best.pt` 是后来继续训到大约第 **448** 轮的门控网（15×15，6×64，搜索 400 次模拟）。克隆仓库就能对弈。
